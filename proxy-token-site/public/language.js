(() => {
  ...
  const translations = new Map(Object.entries({
    ...
    "列出指定标的的活跃期权合约。默认 Alpaca，失败时回退到 ThetaData Value 合约列表。": "List current active option contracts for an underlying from Alpaca. Use the native /v3/option/list/contracts/* routes with a historical date for expired contracts.",
    "期权合约历史 OHLCV K线。默认 ThetaData Value，必要时回退 Alpaca。支持 OCC 或股票代码自动解析，并写入服务端缓存。": "Retrieve canonical 1Min historical OHLCV bars for OCC option contracts. Alpaca fallback is eligible only for ranges starting on/after 2024-02-01 and may be sparse traded activity; earlier or boundary-crossing ranges are ThetaData-only and fail closed if unavailable. The response reports provenance; /v1/options/bars is a legacy alias. Server-side 5Min aggregation is not provided.",
    "期权历史 K 线固定返回 1Min。传入 5Min 等旧参数时会归一化为 1Min；如需其他周期请客户端自行重采样。": "Option history bars return 1Min only. Legacy timeframe values such as 5Min are normalized to 1Min; clients must resample locally. Check provider, providers, and coverage_roles for provenance.",
