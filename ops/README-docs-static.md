...
4. Materialize only the seven public files in `deploy_docs_static.py` plus that
   reviewed script from the committed Git archive into
...
replaces files inside existing directories, and compares host/container hashes.
It neither replaces the mounted directory nor restarts services. A failed local
verification restores the seven files. A repeated apply fails closed.

7. Compare release, host, container and public response hashes for all seven files.
...
If public acceptance fails, restore the seven files from `<release>/rollback`
