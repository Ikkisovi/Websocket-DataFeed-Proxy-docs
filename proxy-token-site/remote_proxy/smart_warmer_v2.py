...
def parse_args():
    ...
    p.add_argument("--tickers-file", type=str, default="", help="Path to JSON or Lean universe_selection.py")
    p.add_argument("--all-endpoints", action="store_true", help="Hit all endpoints instead of just baseline")
    return p.parse_args()
...
def mine_audit(audit_file: str, days: int, limit: int) -> list[tuple[str, str, str]]:
    ...
    with open(audit_file) as f:
        for line in f:
            ...
            # Skip internal load-test users so test traffic doesn't poison the
            # "popular tickers" list. Test users are provisioned with usernames
            # prefixed "perftest_" by provision_test_users.py.
            uid = ev.get("user_id", "")
            if isinstance(uid, str) and uid.startswith("perftest_"):
                continue
...
def load_baseline_tickers(args) -> list[str]:
    if args.tickers_file:
        file_path = Path(args.tickers_file)
        if not file_path.exists():
            print(f"[tickers] {file_path} not found")
            return []
        ...
        if file_path.suffix == ".py":
            ...
            if m:
                try:
                    ...
                    print(f"[tickers] Extracted {len(tickers)} tickers from Python file")
                    return tickers
            ...
            if isinstance(data, list):
                print(f"[tickers] Extracted {len(data)} tickers from JSON list")
                return data
            sp500  = data.get("sp500", [])
            ndx100 = data.get("nasdaq100", [])
            etfs   = data.get("etfs", [])
            combined = list(dict.fromkeys(sp500 + ndx100 + etfs))
            print(f"[tickers] Loaded {len(combined)} tickers from JSON dict")
            return combined
...
async def warm_one(session: aiohttp.ClientSession, proxy: str, token: str,
    ...
    if endpoint in ("/v1/stock/history/trade_quote",):
        ...
        # Reconciliation: force SIP-served refresh so any cache entry
        # previously written from a free-key (IEX) routing decision gets
        # overwritten with the authoritative SIP version overnight.
        if endpoint in ("/v1/history/bars", "/v1/history/news"):
...
async def run(args):
    ...
    baseline_tickers = load_baseline_tickers(args)
    ...
    for sym in baseline_tickers:
        if args.all_endpoints:
            # Hit EVERYTHING for this ticker
            contracts = all_contracts.get(sym, [])
            joined_contracts = ",".join(contracts) if contracts else ""

            for tf in BAR_TIMEFRAMES:
                days_back = args.baseline_days if tf == "1Day" else TF_DAYS.get(tf, 30)
                start = (today - timedelta(days=days_back)).isoformat()
                work.append(("/v1/history/bars", sym, tf, start, end_date))
                if joined_contracts:
                    work.append(("/v1/history/options/bars", joined_contracts, tf, start, end_date))
            
            start_7 = (today - timedelta(days=7)).isoformat()
            work.append(("/v1/stock/history/trade_quote", sym, "", start_7, end_date))
            work.append(("/v1/history/news", sym, "", start_7, end_date))
            if joined_contracts:
                work.append(("/v1/history/options/trade_quote", joined_contracts, "", start_7, end_date))
                work.append(("/v1/options/eod", joined_contracts, "", start_7, end_date))
            ...
            if contracts:
                ...
                if expiry_date:
                    work.append(("/v1/options/snapshots/expiry", sym, expiry_date, "", ""))
        else:
            ...
            if joined_contracts:
                for tf in BAR_TIMEFRAMES:
                    ...
                    # We can't perfectly check hot_pairs for joined_contracts easily if it's mixed, but baseline warms it anyway
                    days_back = args.baseline_days if tf == "1Day" else TF_DAYS.get(tf, 30)
                    days_back = min(days_back, 30)
                    start = (today - timedelta(days=days_back)).isoformat()
                    work.append((ep, joined_contracts, tf, start, end_date))
